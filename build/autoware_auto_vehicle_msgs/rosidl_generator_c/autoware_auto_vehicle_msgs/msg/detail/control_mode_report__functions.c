  ...
  autoware_auto_vehicle_msgs__msg__ControlModeReport__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    autoware_auto_vehicle_msgs__msg__ControlModeReport * data =
      (autoware_auto_vehicle_msgs__msg__ControlModeReport *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!autoware_auto_vehicle_msgs__msg__ControlModeReport__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          autoware_auto_vehicle_msgs__msg__ControlModeReport__fini(&output->data[i]);
        }
        return false;
      }
    }
