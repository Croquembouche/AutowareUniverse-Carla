// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from v2x_msg:msg/AccelSets.idl
// generated code does not contain a copyright notice
#include "v2x_msg/msg/detail/accel_sets__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>


// Include directives for member types
// Member `angle`
#include "v2x_msg/msg/detail/acceleration_set4_way__functions.h"
// Member `confidenceset`
#include "v2x_msg/msg/detail/confidence_set__functions.h"

bool
v2x_msg__msg__AccelSets__init(v2x_msg__msg__AccelSets * msg)
{
  if (!msg) {
    return false;
  }
  // angle
  if (!v2x_msg__msg__AccelerationSet4Way__init(&msg->angle)) {
    v2x_msg__msg__AccelSets__fini(msg);
    return false;
  }
  // certaccelthres
  // yawratecon
  // hozaccelcon
  // confidenceset
  if (!v2x_msg__msg__ConfidenceSet__init(&msg->confidenceset)) {
    v2x_msg__msg__AccelSets__fini(msg);
    return false;
  }
  return true;
}

void
v2x_msg__msg__AccelSets__fini(v2x_msg__msg__AccelSets * msg)
{
  if (!msg) {
    return;
  }
  // angle
  v2x_msg__msg__AccelerationSet4Way__fini(&msg->angle);
  // certaccelthres
  // yawratecon
  // hozaccelcon
  // confidenceset
  v2x_msg__msg__ConfidenceSet__fini(&msg->confidenceset);
}

bool
v2x_msg__msg__AccelSets__are_equal(const v2x_msg__msg__AccelSets * lhs, const v2x_msg__msg__AccelSets * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // angle
  if (!v2x_msg__msg__AccelerationSet4Way__are_equal(
      &(lhs->angle), &(rhs->angle)))
  {
    return false;
  }
  // certaccelthres
  if (lhs->certaccelthres != rhs->certaccelthres) {
    return false;
  }
  // yawratecon
  if (lhs->yawratecon != rhs->yawratecon) {
    return false;
  }
  // hozaccelcon
  if (lhs->hozaccelcon != rhs->hozaccelcon) {
    return false;
  }
  // confidenceset
  if (!v2x_msg__msg__ConfidenceSet__are_equal(
      &(lhs->confidenceset), &(rhs->confidenceset)))
  {
    return false;
  }
  return true;
}

bool
v2x_msg__msg__AccelSets__copy(
  const v2x_msg__msg__AccelSets * input,
  v2x_msg__msg__AccelSets * output)
{
  if (!input || !output) {
    return false;
  }
  // angle
  if (!v2x_msg__msg__AccelerationSet4Way__copy(
      &(input->angle), &(output->angle)))
  {
    return false;
  }
  // certaccelthres
  output->certaccelthres = input->certaccelthres;
  // yawratecon
  output->yawratecon = input->yawratecon;
  // hozaccelcon
  output->hozaccelcon = input->hozaccelcon;
  // confidenceset
  if (!v2x_msg__msg__ConfidenceSet__copy(
      &(input->confidenceset), &(output->confidenceset)))
  {
    return false;
  }
  return true;
}

v2x_msg__msg__AccelSets *
v2x_msg__msg__AccelSets__create()
{
  v2x_msg__msg__AccelSets * msg = (v2x_msg__msg__AccelSets *)malloc(sizeof(v2x_msg__msg__AccelSets));
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(v2x_msg__msg__AccelSets));
  bool success = v2x_msg__msg__AccelSets__init(msg);
  if (!success) {
    free(msg);
    return NULL;
  }
  return msg;
}

void
v2x_msg__msg__AccelSets__destroy(v2x_msg__msg__AccelSets * msg)
{
  if (msg) {
    v2x_msg__msg__AccelSets__fini(msg);
  }
  free(msg);
}


bool
v2x_msg__msg__AccelSets__Sequence__init(v2x_msg__msg__AccelSets__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  v2x_msg__msg__AccelSets * data = NULL;
  if (size) {
    data = (v2x_msg__msg__AccelSets *)calloc(size, sizeof(v2x_msg__msg__AccelSets));
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = v2x_msg__msg__AccelSets__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        v2x_msg__msg__AccelSets__fini(&data[i - 1]);
      }
      free(data);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
v2x_msg__msg__AccelSets__Sequence__fini(v2x_msg__msg__AccelSets__Sequence * array)
{
  if (!array) {
    return;
  }
  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      v2x_msg__msg__AccelSets__fini(&array->data[i]);
    }
    free(array->data);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

v2x_msg__msg__AccelSets__Sequence *
v2x_msg__msg__AccelSets__Sequence__create(size_t size)
{
  v2x_msg__msg__AccelSets__Sequence * array = (v2x_msg__msg__AccelSets__Sequence *)malloc(sizeof(v2x_msg__msg__AccelSets__Sequence));
  if (!array) {
    return NULL;
  }
  bool success = v2x_msg__msg__AccelSets__Sequence__init(array, size);
  if (!success) {
    free(array);
    return NULL;
  }
  return array;
}

void
v2x_msg__msg__AccelSets__Sequence__destroy(v2x_msg__msg__AccelSets__Sequence * array)
{
  if (array) {
    v2x_msg__msg__AccelSets__Sequence__fini(array);
  }
  free(array);
}

bool
v2x_msg__msg__AccelSets__Sequence__are_equal(const v2x_msg__msg__AccelSets__Sequence * lhs, const v2x_msg__msg__AccelSets__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!v2x_msg__msg__AccelSets__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
v2x_msg__msg__AccelSets__Sequence__copy(
  const v2x_msg__msg__AccelSets__Sequence * input,
  v2x_msg__msg__AccelSets__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(v2x_msg__msg__AccelSets);
    v2x_msg__msg__AccelSets * data =
      (v2x_msg__msg__AccelSets *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!v2x_msg__msg__AccelSets__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          v2x_msg__msg__AccelSets__fini(&data[i]);
        }
        free(data);
        return false;
      }
    }
    output->data = data;
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!v2x_msg__msg__AccelSets__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
