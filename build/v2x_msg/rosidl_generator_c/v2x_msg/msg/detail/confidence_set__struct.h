// generated from rosidl_generator_c/resource/idl__struct.h.em
// with input from v2x_msg:msg/ConfidenceSet.idl
// generated code does not contain a copyright notice

#ifndef V2X_MSG__MSG__DETAIL__CONFIDENCE_SET__STRUCT_H_
#define V2X_MSG__MSG__DETAIL__CONFIDENCE_SET__STRUCT_H_

#ifdef __cplusplus
extern "C"
{
#endif

#include <stdbool.h>
#include <stddef.h>
#include <stdint.h>


// Constants defined in the message

// Include directives for member types
// Member 'accelconfidence'
#include "v2x_msg/msg/detail/accel_steer_yaw_rate_confidence__struct.h"
// Member 'speedconfidence'
#include "v2x_msg/msg/detail/speedand_headingand_throttle_confidence__struct.h"
// Member 'posconfidence'
#include "v2x_msg/msg/detail/position_confidence_set__struct.h"

/// Struct defined in msg/ConfidenceSet in the package v2x_msg.
typedef struct v2x_msg__msg__ConfidenceSet
{
  v2x_msg__msg__AccelSteerYawRateConfidence accelconfidence;
  v2x_msg__msg__SpeedandHeadingandThrottleConfidence speedconfidence;
  int64_t timeconfidence;
  v2x_msg__msg__PositionConfidenceSet posconfidence;
  int64_t steerconfidence;
  int64_t headingconfidence;
  int64_t throttleconfidence;
} v2x_msg__msg__ConfidenceSet;

// Struct for a sequence of v2x_msg__msg__ConfidenceSet.
typedef struct v2x_msg__msg__ConfidenceSet__Sequence
{
  v2x_msg__msg__ConfidenceSet * data;
  /// The number of valid items in data
  size_t size;
  /// The number of allocated items in data
  size_t capacity;
} v2x_msg__msg__ConfidenceSet__Sequence;

#ifdef __cplusplus
}
#endif

#endif  // V2X_MSG__MSG__DETAIL__CONFIDENCE_SET__STRUCT_H_
