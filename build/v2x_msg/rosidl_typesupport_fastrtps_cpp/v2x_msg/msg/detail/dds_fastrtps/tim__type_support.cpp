  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = v2x_msg::msg::TIM;
    is_plain =
      (
      offsetof(DataType, dataframes) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _TIM__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_TIM(full_bounded, is_plain, 0);
