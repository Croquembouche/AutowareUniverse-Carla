# generated from rosidl_generator_py/resource/_idl.py.em
# with input from v2x_msg:msg/ConfidenceSet.idl
# generated code does not contain a copyright notice


# Import statements for member types

import rosidl_parser.definition  # noqa: E402, I100


class Metaclass_ConfidenceSet(type):
    """Metaclass of message 'ConfidenceSet'."""

    _CREATE_ROS_MESSAGE = None
    _CONVERT_FROM_PY = None
    _CONVERT_TO_PY = None
    _DESTROY_ROS_MESSAGE = None
    _TYPE_SUPPORT = None

    __constants = {
    }

    @classmethod
    def __import_type_support__(cls):
        try:
            from rosidl_generator_py import import_type_support
            module = import_type_support('v2x_msg')
        except ImportError:
            import logging
            import traceback
            logger = logging.getLogger(
                'v2x_msg.msg.ConfidenceSet')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
            cls._CREATE_ROS_MESSAGE = module.create_ros_message_msg__msg__confidence_set
            cls._CONVERT_FROM_PY = module.convert_from_py_msg__msg__confidence_set
            cls._CONVERT_TO_PY = module.convert_to_py_msg__msg__confidence_set
            cls._TYPE_SUPPORT = module.type_support_msg__msg__confidence_set
            cls._DESTROY_ROS_MESSAGE = module.destroy_ros_message_msg__msg__confidence_set

            from v2x_msg.msg import AccelSteerYawRateConfidence
            if AccelSteerYawRateConfidence.__class__._TYPE_SUPPORT is None:
                AccelSteerYawRateConfidence.__class__.__import_type_support__()

            from v2x_msg.msg import PositionConfidenceSet
            if PositionConfidenceSet.__class__._TYPE_SUPPORT is None:
                PositionConfidenceSet.__class__.__import_type_support__()

            from v2x_msg.msg import SpeedandHeadingandThrottleConfidence
            if SpeedandHeadingandThrottleConfidence.__class__._TYPE_SUPPORT is None:
                SpeedandHeadingandThrottleConfidence.__class__.__import_type_support__()

    @classmethod
    def __prepare__(cls, name, bases, **kwargs):
        # list constant names here so that they appear in the help text of
        # the message class under "Data and other attributes defined here:"
        # as well as populate each message instance
        return {
        }


class ConfidenceSet(metaclass=Metaclass_ConfidenceSet):
    """Message class 'ConfidenceSet'."""

    __slots__ = [
        '_accelconfidence',
        '_speedconfidence',
        '_timeconfidence',
        '_posconfidence',
        '_steerconfidence',
        '_headingconfidence',
        '_throttleconfidence',
    ]

    _fields_and_field_types = {
        'accelconfidence': 'v2x_msg/AccelSteerYawRateConfidence',
        'speedconfidence': 'v2x_msg/SpeedandHeadingandThrottleConfidence',
        'timeconfidence': 'int64',
        'posconfidence': 'v2x_msg/PositionConfidenceSet',
        'steerconfidence': 'int64',
        'headingconfidence': 'int64',
        'throttleconfidence': 'int64',
    }

    SLOT_TYPES = (
        rosidl_parser.definition.NamespacedType(['v2x_msg', 'msg'], 'AccelSteerYawRateConfidence'),  # noqa: E501
        rosidl_parser.definition.NamespacedType(['v2x_msg', 'msg'], 'SpeedandHeadingandThrottleConfidence'),  # noqa: E501
        rosidl_parser.definition.BasicType('int64'),  # noqa: E501
        rosidl_parser.definition.NamespacedType(['v2x_msg', 'msg'], 'PositionConfidenceSet'),  # noqa: E501
        rosidl_parser.definition.BasicType('int64'),  # noqa: E501
        rosidl_parser.definition.BasicType('int64'),  # noqa: E501
        rosidl_parser.definition.BasicType('int64'),  # noqa: E501
    )

    def __init__(self, **kwargs):
        assert all('_' + key in self.__slots__ for key in kwargs.keys()), \
            'Invalid arguments passed to constructor: %s' % \
            ', '.join(sorted(k for k in kwargs.keys() if '_' + k not in self.__slots__))
        from v2x_msg.msg import AccelSteerYawRateConfidence
        self.accelconfidence = kwargs.get('accelconfidence', AccelSteerYawRateConfidence())
        from v2x_msg.msg import SpeedandHeadingandThrottleConfidence
        self.speedconfidence = kwargs.get('speedconfidence', SpeedandHeadingandThrottleConfidence())
        self.timeconfidence = kwargs.get('timeconfidence', int())
        from v2x_msg.msg import PositionConfidenceSet
        self.posconfidence = kwargs.get('posconfidence', PositionConfidenceSet())
        self.steerconfidence = kwargs.get('steerconfidence', int())
        self.headingconfidence = kwargs.get('headingconfidence', int())
        self.throttleconfidence = kwargs.get('throttleconfidence', int())

    def __repr__(self):
        typename = self.__class__.__module__.split('.')
        typename.pop()
        typename.append(self.__class__.__name__)
        args = []
        for s, t in zip(self.__slots__, self.SLOT_TYPES):
            field = getattr(self, s)
            fieldstr = repr(field)
            # We use Python array type for fields that can be directly stored
            # in them, and "normal" sequences for everything else.  If it is
            # a type that we store in an array, strip off the 'array' portion.
            if (
                isinstance(t, rosidl_parser.definition.AbstractSequence) and
                isinstance(t.value_type, rosidl_parser.definition.BasicType) and
                t.value_type.typename in ['float', 'double', 'int8', 'uint8', 'int16', 'uint16', 'int32', 'uint32', 'int64', 'uint64']
            ):
                if len(field) == 0:
                    fieldstr = '[]'
                else:
                    assert fieldstr.startswith('array(')
                    prefix = "array('X', "
                    suffix = ')'
                    fieldstr = fieldstr[len(prefix):-len(suffix)]
            args.append(s[1:] + '=' + fieldstr)
        return '%s(%s)' % ('.'.join(typename), ', '.join(args))

    def __eq__(self, other):
        if not isinstance(other, self.__class__):
            return False
        if self.accelconfidence != other.accelconfidence:
            return False
        if self.speedconfidence != other.speedconfidence:
            return False
        if self.timeconfidence != other.timeconfidence:
            return False
        if self.posconfidence != other.posconfidence:
            return False
        if self.steerconfidence != other.steerconfidence:
            return False
        if self.headingconfidence != other.headingconfidence:
            return False
        if self.throttleconfidence != other.throttleconfidence:
            return False
        return True

    @classmethod
    def get_fields_and_field_types(cls):
        from copy import copy
        return copy(cls._fields_and_field_types)

    @property
    def accelconfidence(self):
        """Message field 'accelconfidence'."""
        return self._accelconfidence

    @accelconfidence.setter
    def accelconfidence(self, value):
        if __debug__:
            from v2x_msg.msg import AccelSteerYawRateConfidence
            assert \
                isinstance(value, AccelSteerYawRateConfidence), \
                "The 'accelconfidence' field must be a sub message of type 'AccelSteerYawRateConfidence'"
        self._accelconfidence = value

    @property
    def speedconfidence(self):
        """Message field 'speedconfidence'."""
        return self._speedconfidence

    @speedconfidence.setter
    def speedconfidence(self, value):
        if __debug__:
            from v2x_msg.msg import SpeedandHeadingandThrottleConfidence
            assert \
                isinstance(value, SpeedandHeadingandThrottleConfidence), \
                "The 'speedconfidence' field must be a sub message of type 'SpeedandHeadingandThrottleConfidence'"
        self._speedconfidence = value

    @property
    def timeconfidence(self):
        """Message field 'timeconfidence'."""
        return self._timeconfidence

    @timeconfidence.setter
    def timeconfidence(self, value):
        if __debug__:
            assert \
                isinstance(value, int), \
                "The 'timeconfidence' field must be of type 'int'"
            assert value >= -9223372036854775808 and value < 9223372036854775808, \
                "The 'timeconfidence' field must be an integer in [-9223372036854775808, 9223372036854775807]"
        self._timeconfidence = value

    @property
    def posconfidence(self):
        """Message field 'posconfidence'."""
        return self._posconfidence

    @posconfidence.setter
    def posconfidence(self, value):
        if __debug__:
            from v2x_msg.msg import PositionConfidenceSet
            assert \
                isinstance(value, PositionConfidenceSet), \
                "The 'posconfidence' field must be a sub message of type 'PositionConfidenceSet'"
        self._posconfidence = value

    @property
    def steerconfidence(self):
        """Message field 'steerconfidence'."""
        return self._steerconfidence

    @steerconfidence.setter
    def steerconfidence(self, value):
        if __debug__:
            assert \
                isinstance(value, int), \
                "The 'steerconfidence' field must be of type 'int'"
            assert value >= -9223372036854775808 and value < 9223372036854775808, \
                "The 'steerconfidence' field must be an integer in [-9223372036854775808, 9223372036854775807]"
        self._steerconfidence = value

    @property
    def headingconfidence(self):
        """Message field 'headingconfidence'."""
        return self._headingconfidence

    @headingconfidence.setter
    def headingconfidence(self, value):
        if __debug__:
            assert \
                isinstance(value, int), \
                "The 'headingconfidence' field must be of type 'int'"
            assert value >= -9223372036854775808 and value < 9223372036854775808, \
                "The 'headingconfidence' field must be an integer in [-9223372036854775808, 9223372036854775807]"
        self._headingconfidence = value

    @property
    def throttleconfidence(self):
        """Message field 'throttleconfidence'."""
        return self._throttleconfidence

    @throttleconfidence.setter
    def throttleconfidence(self, value):
        if __debug__:
            assert \
                isinstance(value, int), \
                "The 'throttleconfidence' field must be of type 'int'"
            assert value >= -9223372036854775808 and value < 9223372036854775808, \
                "The 'throttleconfidence' field must be an integer in [-9223372036854775808, 9223372036854775807]"
        self._throttleconfidence = value
