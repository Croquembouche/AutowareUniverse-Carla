...
bool v2x_msg__msg__region_point_set__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("v2x_msg.msg._region_point_set.RegionPointSet", full_classname_dest, 44) == 0);
  }
  v2x_msg__msg__RegionPointSet * ros_message = _ros_message;
  {  // anchor
  ...
  {  // nodelist
    ...
    if (!v2x_msg__msg__RegionOffsets__Sequence__init(&(ros_message->nodelist), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create v2x_msg__msg__RegionOffsets__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    v2x_msg__msg__RegionOffsets * dest = ros_message->nodelist.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!v2x_msg__msg__region_offsets__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
{
  /* NOTE(esteve): Call constructor of RegionPointSet */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("v2x_msg.msg._region_point_set");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "RegionPointSet");
  ...
  {  // nodelist
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      PyObject * pyitem = v2x_msg__msg__region_offsets__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
