...
# -------------------LiDAR Sensor----------------------------
    def createPCL2msg(self, data):
        ...
        return msg
    def lidarSensorCallback_16(self, data):
        msg = self.createPCL2msg(data)
        # self.get_logger().info('Publishing: Lidar Data')
        self.lidarpublisher_16.publish(msg) 
        # change msgcnt
        self.msgcntIncrem() 
    def addLidarSensor_16(self, x=0,y=0,z=1.7, pitch=0,yaw=0,row=0, channels="16", pps="300000", rot_freq="50", up_fov="25", low_fov="-25", range="20", dropoff_rate="0.0", dropoff_limit="1.0", dropoff_zero="0.0"):
        # creating a publisher
        self.lidarpublisher_16 = self.create_publisher(PointCloud2, 'lidar_16', 10)
        ...
        lidar_sen.listen(lambda point_cloud: self.lidarSensorCallback_16(point_cloud))
    def lidarSensorCallback_32(self, data):
        msg = self.createPCL2msg(data)
        # self.get_logger().info('Publishing: Lidar Data')
        self.lidarpublisher_32.publish(msg) 
        # change msgcnt
        self.msgcntIncrem() 
    def addLidarSensor_32(self, x=0,y=0,z=1.7, pitch=0,yaw=0,row=0, channels="32", pps="1200000", rot_freq="50", up_fov="25", low_fov="-25", range="20", dropoff_rate="0.0", dropoff_limit="1.0", dropoff_zero="0.0"):
        # creating a publisher
        self.lidarpublisher_32 = self.create_publisher(PointCloud2, 'lidar_32', 10)
        # finding the lidar bp
        lidar_bp = self.world.get_blueprint_library().find('sensor.lidar.ray_cast')
        # settings the lidar param
        lidar_bp.set_attribute('channels', channels)
        ...
        self.sensors.append(lidar_sen)
        # add a callback function to publish data via ros2
        lidar_sen.listen(lambda point_cloud: self.lidarSensorCallback_32(point_cloud))
    def lidarSensorCallback_64(self, data):
        msg = self.createPCL2msg(data)
        # self.get_logger().info('Publishing: Lidar Data')
        self.lidarpublisher_64.publish(msg) 
        # change msgcnt
        self.msgcntIncrem() 
    def addLidarSensor_64(self, x=0,y=0,z=1.7, pitch=0,yaw=0,row=0, channels="64", pps="2200000", rot_freq="50", up_fov="25", low_fov="-25", range="20", dropoff_rate="0.0", dropoff_limit="1.0", dropoff_zero="0.0"):
        # creating a publisher
        self.lidarpublisher_64 = self.create_publisher(PointCloud2, 'lidar_64', 10)
        # finding the lidar bp
        lidar_bp = self.world.get_blueprint_library().find('sensor.lidar.ray_cast')
        # settings the lidar param
        lidar_bp.set_attribute('channels', channels)
        ...
        self.sensors.append(lidar_sen)
        # add a callback function to publish data via ros2
        lidar_sen.listen(lambda point_cloud: self.lidarSensorCallback_64(point_cloud))
        # lidar_sen.listen(lambda point_cloud: point_cloud.save_to_disk('/home/carla/Github/C-V2X-Autoware-Carla/src/vehicle/ren_test/%.6d.ply' % point_cloud.frame))
...
def main(args=None):
    ...
    # add lidar sensor
    demo_veh.addLidarSensor_16()
    demo_veh.addLidarSensor_32()
    demo_veh.addLidarSensor_64()
