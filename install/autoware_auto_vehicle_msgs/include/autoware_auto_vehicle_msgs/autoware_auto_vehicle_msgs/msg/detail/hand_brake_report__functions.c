  ...
  autoware_auto_vehicle_msgs__msg__HandBrakeReport__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    autoware_auto_vehicle_msgs__msg__HandBrakeReport * data =
      (autoware_auto_vehicle_msgs__msg__HandBrakeReport *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!autoware_auto_vehicle_msgs__msg__HandBrakeReport__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          autoware_auto_vehicle_msgs__msg__HandBrakeReport__fini(&output->data[i]);
        }
        return false;
      }
    }
