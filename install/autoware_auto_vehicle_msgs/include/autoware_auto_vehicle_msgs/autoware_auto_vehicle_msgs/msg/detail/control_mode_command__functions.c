  ...
  autoware_auto_vehicle_msgs__msg__ControlModeCommand__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    autoware_auto_vehicle_msgs__msg__ControlModeCommand * data =
      (autoware_auto_vehicle_msgs__msg__ControlModeCommand *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!autoware_auto_vehicle_msgs__msg__ControlModeCommand__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          autoware_auto_vehicle_msgs__msg__ControlModeCommand__fini(&output->data[i]);
        }
        return false;
      }
    }
