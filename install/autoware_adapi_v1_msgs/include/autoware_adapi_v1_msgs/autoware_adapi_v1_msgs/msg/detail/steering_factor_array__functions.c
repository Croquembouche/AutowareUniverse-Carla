  ...
  autoware_adapi_v1_msgs__msg__SteeringFactorArray__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    autoware_adapi_v1_msgs__msg__SteeringFactorArray * data =
      (autoware_adapi_v1_msgs__msg__SteeringFactorArray *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!autoware_adapi_v1_msgs__msg__SteeringFactorArray__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          autoware_adapi_v1_msgs__msg__SteeringFactorArray__fini(&output->data[i]);
        }
        return false;
      }
    }
