  ...
  autoware_adapi_v1_msgs__msg__MrmState__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    autoware_adapi_v1_msgs__msg__MrmState * data =
      (autoware_adapi_v1_msgs__msg__MrmState *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!autoware_adapi_v1_msgs__msg__MrmState__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          autoware_adapi_v1_msgs__msg__MrmState__fini(&output->data[i]);
        }
        return false;
      }
    }
